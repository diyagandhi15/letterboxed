...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {   
        // if new word and first character     
        if (wordNum == 0 && charNum == 0) {
            return true; 
        }

        String wordString = words[wordNum];
        String newWord = wordString + letter;
    
        if (charNum == 0) {
            if (letter.equals(lastLetter(words[wordNum - 1]))){
                return true;
            } else {
                return false;
            }
        ...
        if (this.onSameSide(letter, lastLetter(words[wordNum])) == false
            && this.alreadyUsed(newWord) == false
            && dictionary.hasString(newWord)) {
            return true;
        } 
        else {
            return false;
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        ...
        int length = this.words[wordNum].length();

        if (this.allLettersUsed() && length >= 3 && dictionary.hasFullWord(this.words[wordNum])) {
        ...
        for (int index = 0; index < this.letters.length; index++) {
            String tempLetter = letters[index];
            if (this.isValid(tempLetter, wordNum, charNum)) {
                this.addLetter((tempLetter), wordNum);
	                if (this.solveRB(wordNum, charNum + 1, maxWords)) {
		                return true;
                    } 
	                else if (length >= 3 && dictionary.hasFullWord(this.words[wordNum])){ 
    ...
    public static void main(String[] args) {
        ...
        // System.out.println(test.isValid("s", 0, 4));
        // System.out.println(test.isValid("y", 0, 4));
        // System.exit(0);
